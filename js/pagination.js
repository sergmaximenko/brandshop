			...
			if (none == 1) {
				...
				for (let i = 0; i < notes.length; i++) {
					...
					item.onclick = function (el) {
						...
						sessionStorage.setItem("item", JSON.stringify(notes[i]));
						//						loadItem(notes[i].id + 'ID')
						route(`item?${notes[i].id}`)
					}
					...
					for (let a = 0; a < notes[i].size.length; a++) {
						...
						div.onclick = function () {
							if (bas.length == 0) {
								...
								for (let x = 0; x < bas.length; x++) {
									if (bas[x].id == notes[i].id) {
										break;
									} else if (x == bas.length - 1) {
										bas.push(notes[i]);
										localStorage.setItem("bas", JSON.stringify(bas));
									}
								}
