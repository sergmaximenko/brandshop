function pageItem() {
	...
	let tubItem1 = document.querySelectorAll(".info div");
	let tubItem = [];
	...
	for (let i = 0; i < tubItem1.length; i++) {
		if (tubItem1[i].parentNode.className == 'info') {
			tubItem.push(tubItem1[i]);
		}
	}

	let shadow = [];
	for (let i = 0; i < imgTub.length; i++) {
		shadow.push(imgTub[i].parentNode.children[1])
	...
	for (let i = 0; i < tubSize.length; i++) {
		tubSize[i].onclick = function () {
			...
			setTimeout(function () {
				addBasketOne.classList.remove('display0');
			}, 250);
			addBasketTwo.classList.remove('opacity1');
			setTimeout(function () {
				addBasketTwo.classList.add('display0');
			}, 250);
	...
	addBasket.parentNode.onclick = function () {
		...
		setTimeout(function () {
			tuta.classList.add('display0');
		}, 250)
		addBasketTwo.classList.add('opacity1');

		setTimeout(function () {
			addBasketTwo.classList.toggle('display0');
		}, 250)
		...
		span[0].onclick = function () {
			if (parseInt(span[1].textContent) == 1) {
				...
				setTimeout(function () {
					tuta.classList.remove('display0');
				}, 250);
				addBasketTwo.classList.remove('opacity1');
				setTimeout(function () {
					addBasketTwo.classList.toggle('display0');
